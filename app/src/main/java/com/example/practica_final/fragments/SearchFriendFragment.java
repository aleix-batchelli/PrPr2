        ...
        JsonObjectRequest stringRequest = new JsonObjectRequest(Request.Method.GET, url, jsonObject, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    System.out.println("XXXMARC");
                    User[] allUsers = convertJSONArrayToUsers(users);
                    System.out.println("AAAMARC");

                    friendAdapter.setUsers(allUsers);  // Configurar los usuarios en el adaptador
                    System.out.println("BBBMARC");

                    friendAdapter.notifyDataSetChanged();  // Notificar al adaptador que los datos han cambiado
                    System.out.println("CCCMARC");
    ...
    private User[] convertJSONArrayToUsers(JSONArray jsonArray) {
        ...
        try {
            for (int i = 0; i < length; i++) {
                ...
                // Aquí debes parsear los campos del objeto JSON para crear una instancia de User y agregarla al array
                // Por ejemplo:
